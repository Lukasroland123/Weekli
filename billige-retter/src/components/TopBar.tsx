...
export default function TopBar() {
  const { state, setPersons, setSelectedChains, setPrisLogik, setTagFilter, setSearchQuery, setSearchType } = useApp();
  ...
  return (
    <div ref={containerRef} className="bg-white border-b border-gray-100 px-4 pt-3 sticky top-0 z-40">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="flex bg-gray-50 border border-gray-100 rounded-xl overflow-hidden">
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-semibold transition-colors ${
              state.prisLogik === "kilopris" ? "bg-green-600 text-white" : "text-gray-500"
            }`}
          >
            Kilopris
          </button>
      ...
      <div className="flex gap-2 py-2 overflow-x-auto scrollbar-none">
        <TagChip label="Alle" active={state.tagFilter === null} onClick={() => setTagFilter(null)} />
        {TAG_OPTIONS.map(({ value, label }) => (
          <TagChip
            key={value}
            label={label}
            active={state.tagFilter === value}
            onClick={() => setTagFilter(state.tagFilter === value ? null : value)}
          />
        ))}
      </div>
    </div>
